...
(function () {
  ...
  function getValue(state, key) {
    ...
    if (alias && Object.prototype.hasOwnProperty.call(state, alias)) return state[alias];
    return "";
  }
  ...
  function normalizeValueForPrint(v, { type = null } = {}) {
    ...
    if (typeof v === "object") {
      ...
      if (Object.prototype.hasOwnProperty.call(v, "label")) return String(v.label);
  ...
  function renderLineField(label, value, { wide = false, multiline = false } = {}) {
  ...
  function renderRadioChoices(value, options) {
    ...
    return safeOptions
      .map((opt) => renderCheckbox(v === String(opt.value), opt.label || opt.value))
      .join("");
  }
  ...
  function enrollmentPage1(state) {
    ...
    const parent1Block = [
      renderLineField("Parent/Guardian (1) name", normalizeValueForPrint(getValue(state, "parent1_name"))),
      renderLineField("Email", normalizeValueForPrint(getValue(state, "parent1_email"))),
      renderLineField(
        "Home address",
      ...
      renderLineField("Postal code", normalizeValueForPrint(getValue(state, "parent1_postal_code"))),
      renderLineField("Cell and home #", normalizeValueForPrint(getValue(state, "parent1_phones"))),
      renderLineField(
        "Work/School address",
      ...
      renderLineField("Work/School phone #", normalizeValueForPrint(getValue(state, "parent1_work_phone"))),
    ].join("");

    const parent2Block = [
      renderLineField("Parent/Guardian (2) name", normalizeValueForPrint(getValue(state, "parent2_name"))),
      renderLineField("Email", normalizeValueForPrint(getValue(state, "parent2_email"))),
      renderLineField(
        "Home address",
      ...
      renderLineField("Postal code", normalizeValueForPrint(getValue(state, "parent2_postal_code"))),
      renderLineField("Cell and home #", normalizeValueForPrint(getValue(state, "parent2_phones"))),
      renderLineField(
        "Work/School address",
      ...
      renderLineField("Work/School phone #", normalizeValueForPrint(getValue(state, "parent2_work_phone"))),
    ].join("");

    const doctorBlock = [
      renderLineField("Doctor’s name", normalizeValueForPrint(getValue(state, "doctor_name"))),
      renderLineField("Phone #", normalizeValueForPrint(getValue(state, "doctor_phone"))),
      renderLineField(
        "Doctor’s address",
      ...
      renderLineField("Postal code", normalizeValueForPrint(getValue(state, "doctor_postal_code"))),
    ].join("");

    const allergiesBlock = [
      renderLineField("Does your child have any allergies?", normalizeValueForPrint(getValue(state, "child_allergies")), {
        multiline: true,
        wide: true,
      }),
      renderLineField(
    ...
    const emergencyBlock = [
      ...
      renderLineField(
        ...
        normalizeValueForPrint(getValue(state, "emergency_contact_relationship")),
      ),
      renderLineField(
        "Day time phone #",
    ...
    const pickups = normalizeValueForPrint(getValue(state, "authorized_pickups"));
      ...
      <div class="grasp-section">
        ...
        <div class="grasp-signatures">
          ${renderLineField(
            ...
            normalizeValueForPrint(getValue(state, "parent_full_name_signature")),
            { wide: true },
          )}
          ${renderLineField(
            "Date",
            normalizeValueForPrint(getValue(state, "signature_date"), { type: "date" }),
          )}
      ...
      <div class="grasp-section">
        ...
        <div class="grasp-policy">
          ...
          <div class="grasp-policy-text">
            ...
            by the staff of the child care centre at the time of the incident.
          </div>
          ...
          <div class="grasp-field-grid">
            ...
            <div class="grasp-field-grid grasp-2col">
              ...
              ${renderLineField(
                ...
                normalizeValueForPrint(getValue(state, "last_medical_exam_date"), { type: "date" }),
              )}
        ...
        <div class="grasp-policy">
          ...
          <div class="grasp-choices">
            ${renderRadioChoices(normalizeValueForPrint(getValue(state, "water_play_consent")), [
        ...
        <div class="grasp-policy">
          ...
          <div class="grasp-choices">
            ${renderRadioChoices(normalizeValueForPrint(getValue(state, "hand_sanitizer_consent")), [
              { value: "yes", label: "I consent" },
              { value: "no", label: "I do not consent" },
            ])}
          </div>
      ...
      <div class="grasp-section">
        ...
        <div class="grasp-field-grid grasp-2col">
          ...
          ${renderLineField(
            ...
            normalizeValueForPrint(getValue(state, "child_talking_comprehending")),
          )}
        </div>
        ...
        <div class="grasp-policy">
          ...
          <div class="grasp-choices">
            ${renderRadioChoices(normalizeValueForPrint(getValue(state, "arrival_departure_ack")), [
  ...
  function enrollmentPage4(state) {
    ...
    const infoSharing = normalizeValueForPrint(getValue(state, "info_sharing_consent"));
    const travelConsent = normalizeValueForPrint(getValue(state, "travel_consent"));
    const photoConsent = normalizeValueForPrint(getValue(state, "photo_media_consent"));
        ...
        <div class="grasp-policy">
          ...
          <div class="grasp-choices">
            ${renderRadioChoices(photoConsent, [
              ...
              { value: "limited", label: "Limited use (centre only, internal projects)" },
  ...
  function enrollmentPage5(state) {
    ...
    const safeArrival = normalizeValueForPrint(getValue(state, "safe_arrival_ack"));
    const beforeSchool = normalizeValueForPrint(getValue(state, "before_school_program_ack"));
    const sunscreenProvidedBy = normalizeValueForPrint(getValue(state, "sunscreen_provided_by"));
    const sunscreenAssist = normalizeValueForPrint(getValue(state, "sunscreen_assistance_consent"));
        ...
        <div class="grasp-policy">
          ...
          <div class="grasp-choices">
            ${renderRadioChoices(beforeSchool, [
              { value: "yes", label: "My child may NOT attend before school program daily and may be dropped off directly at school." },
          ...
          <div class="grasp-subsection">
            ...
            <div class="grasp-choices">
              ${renderRadioChoices(sunscreenProvidedBy, [
                ...
                { value: "parent", label: "I will provide my child’s sunscreen (cream only)" },
              ])}
            </div>
          </div>
          ...
          <div class="grasp-subsection">
            ...
            <div class="grasp-choices">
              ${renderRadioChoices(sunscreenAssist, [
                { value: "yes", label: "GRASP may assist my child in the application of sunscreen if necessary" },
          ...
          <div class="grasp-subsection">
            ...
            <div class="grasp-choices">
              ${renderRadioChoices(sunSafety, [
                { value: "yes", label: "I understand I must send my child with a water bottle and hat each day during July and August" },
                { value: "no", label: "I do not agree" },
              ])}
            </div>
          </div>
  ...
  function buildWaitlistPrintHtml(state, config) {
    ...
    const childName = normalizeValueForPrint(getValue(state, "child_name"));
    const birthDate = normalizeValueForPrint(getValue(state, "child_birth_date"), { type: "date" });
    const dateCareNeeded = normalizeValueForPrint(getValue(state, "date_care_needed"), { type: "date" });
    const dateApplied = normalizeValueForPrint(getValue(state, "date_applied"), { type: "date" });

    const address = normalizeValueForPrint(getValue(state, "parent1_home_street"));
    ...
    const postal = normalizeValueForPrint(getValue(state, "parent1_postal_code"));
    ...
    const p1 = [
      renderLineField("Parent 1 name (Guardian 1)", normalizeValueForPrint(getValue(state, "parent1_name"))),
      renderLineField("Email address", normalizeValueForPrint(getValue(state, "parent1_email"))),
      renderLineField("Work phone #", normalizeValueForPrint(getValue(state, "parent1_work_phone"))),
      renderLineField("Cell phone #", normalizeValueForPrint(getValue(state, "parent1_cell_phone"))),
    ].join("");

    const p2 = [
      renderLineField("Parent 2 name (Guardian 2)", normalizeValueForPrint(getValue(state, "parent2_name"))),
      renderLineField("Email address", normalizeValueForPrint(getValue(state, "parent2_email"))),
      renderLineField("Work phone #", normalizeValueForPrint(getValue(state, "parent2_work_phone"))),
      renderLineField("Cell phone #", normalizeValueForPrint(getValue(state, "parent2_cell_phone"))),
    ].join("");
    ...
    const summerField = config && findField(config, "interested_summer_camp_only");
    const schoolYearField = config && findField(config, "interested_school_year_only");

    const subsidyChoices = subsidyField
      ? renderRadioChoices(normalizeValueForPrint(getValue(state, "child_subsidy_status")), subsidyField.options)
      : renderRadioChoices(normalizeValueForPrint(getValue(state, "child_subsidy_status")), [
          { value: "has_subsidy", label: "Has subsidy in place" },
          { value: "on_subsidy_wait_list", label: "Is on the Subsidy wait list" },
          { value: "paying_full_fee", label: "Will be paying full fee" },
        ]);

    const siblingChoices = siblingField
      ? renderRadioChoices(normalizeValueForPrint(getValue(state, "has_sibling_at_grasp")), siblingField.options)
      : renderRadioChoices(normalizeValueForPrint(getValue(state, "has_sibling_at_grasp")), [
          { value: "YES", label: "Yes" },
          { value: "NO", label: "No" },
        ]);

    const summerChoices = summerField
      ? renderRadioChoices(normalizeValueForPrint(getValue(state, "interested_summer_camp_only")), summerField.options)
      : renderRadioChoices(normalizeValueForPrint(getValue(state, "interested_summer_camp_only")), [
          { value: "YES", label: "Yes" },
          { value: "NO", label: "No" },
        ]);

    const schoolYearChoices = schoolYearField
      ? renderRadioChoices(normalizeValueForPrint(getValue(state, "interested_school_year_only")), schoolYearField.options)
      : renderRadioChoices(normalizeValueForPrint(getValue(state, "interested_school_year_only")), [
          { value: "YES", label: "Yes" },
          { value: "NO", label: "No" },
        ]);
    ...
    const currentlyDaycare = normalizeValueForPrint(getValue(state, "currently_attends_daycare"));
    const currentlySchool = normalizeValueForPrint(getValue(state, "currently_attending_school"));
    const willAttendWhen = normalizeValueForPrint(getValue(state, "will_attend_when_require_care"));

    const allergies = normalizeValueForPrint(getValue(state, "allergies_special_needs"));
