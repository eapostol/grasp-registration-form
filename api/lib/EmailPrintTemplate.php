...
private static function renderContentBlocks(string $kind, array $blocks, array $data): string
    {
        ...
        foreach ($blocks as $block) {
            ...
            // Enrollment PDF: Water Play & Hand Sanitizer
            // TCPDF can render accidental leading <br> (often from nl2br() or source formatting) as
            // extra vertical space ABOVE the authorization headings. Strip only for these two blocks.
            if ($kind === 'pdf') {
                if (preg_match('/AUTHORIZATION\s+FOR\s+RECREATIONAL\s+WATER\s+PLAY/i', $html)
                    || preg_match('/AUTHORIZATION\s+FOR\s+THE\s+USE\s+OF\s+HAND\s+SANITIZER/i', $html)) {
    ...
    if (is_array($parts) && count($parts) >= 3) {
        ...
        $html = '<div style="text-align:justify; margin:0; padding:0; line-height:1.15;">' . $p1 . '</div>'
            ...
            if ($kind === 'pdf') {
                ...
                // Enrollment PDF: remove one "line-height" of space above Water Play & Hand Sanitizer headings
                // by tightening ONLY the row_full top padding for these specific policy blocks.
                // Keep title->paragraph spacing intact (handled inside the block HTML), and do not affect the 75/25 consent rows.
                $isWaterPlayAuthHeading = ($t === 'AUTHORIZATION FOR RECREATIONAL WATER PLAY');
                $isHandSanitizerAuthHeading = ($t === 'AUTHORIZATION FOR THE USE OF HAND SANITIZER');
                ...
                if ($isMedicationHeading) {
                    ...
                    $row = str_replace('padding:7px 10px;', 'padding:1px 10px 1px 10px;', $row);
                } elseif ($isWaterPlayAuthHeading || $isHandSanitizerAuthHeading) {
                    // Tighten only the top padding so headings sit higher, while preserving the space above the next border line.
                    $row = str_replace('padding:7px 10px;', 'padding:0px 10px 7px 10px;', $row);
                }
            }
...
HTML;
                    ...
                    $arrivalPolicyHtmlPdf = '<span style="line-height:1.0;">I always agree to accompany my child <strong><u>to and from</u></strong> GRASP classroom and notify staff verbally  <strong><u>upon arrival and departure</u></strong>. I understand that is my responsibility to inform all pick up and drop off persons of this policy and ensure they make verbal contact with the staff. In the event that my child is not accompanied into GRASP facilities by an adult, I understand GRASP has no legal responsibility for the safe arrival of my named child. Failure to inform staff of arrival and departure may result in notifying the authorities. Children’s arrival to GRASP from Greenland PS will be from the designated dismissal door of the school for each class. A GRASP staff member will await outside with all other pick up persons for children to be dismissed from school. If children are not in attendance at GRASP, parents must notify staff by 2:30 pm by email or call. I understand it is not the school responsibility to communicate with GRASP about my child’s attendance.</span>';
...
HTML;
                    // PDF-only: tighten policy paragraph spacing without affecting email layout
                    $disclosureHtmlPdf = '<span style="line-height:1.0;"><em>Consent for sharing information among professionals involved in a child’s day enhances educational and family support.</em><br/><br/>Consent for sharing information is a necessary legal and ethical practice and must be obtained in order to share any information. To provide quality care for children, there are times when it is appropriate for the Childcare Centre, the School, Toronto Children’s Services to exchange information. The kind of information shared may include, but is not limited to, matters involving attendance, illness or transportation etc.  I hereby consent to reciprocal exchange of information about my child between the Centre GRASP / School and/or Toronto Children’s Services.</span>';
...
HTML;
                    // PDF-only: tighten policy paragraph spacing without affecting email layout
                    $travelHtmlPdf = '<span style="line-height:1.0;">I hereby give consent for my child to leave the premises of GRASP under the qualified staff\'s supervision to participate in daily outings, trips to parks, playgrounds, school and libraries that can be reached without public or other motorized transportation. This may occur from time to time with or without prior notice and shall be deemed normal daily activity. I understand that notices will be sent home with consent forms for special trips and events, which involve public or other motorized transportation, swimming off premises. I further understand, the child care center program plans age-appropriate activities in order to keep the children engaged and from time to time may engage in some age-appropriate risky play activities as promoted in childhood development. In order to fully appreciate the program and give all the children the equal opportunity to participate in the plan activities, it is expected and highly recommended all children be in care no later than 9:30 am on non-instructional days such as P.A. Day and summer camp. Any community outing or walks will not depart prior to 9:30 am. Once the group leaves the center for a walk, community outing or field trip staff is not permitted, under any circumstances, to release or accept your child. You must drop off or pick up your child before or after the outing on Greenland property. Children will only be accepted in their designated classes for ratio and safety purposes.</span>';
...
HTML : '');
                    ...
                    if ($kind === 'pdf') {
                        ...
                        $rowsA[] = '<tr><td width="100%" style="border-top:' . $b . '; padding:' . $pad . '; vertical-align:top; text-align:justify;">' . $disclosureHtmlPdf . '</td></tr>';
                    } else {
                    ...
                    if ($kind === 'pdf') {
                        ...
                        $rowsB[] = '<tr><td width="100%" style="border-top:' . $b . '; padding:' . $pad . '; text-align:justify;">' . $travelHtmlPdf . '</td></tr>';
                    } else {
...
$content = self::renderSections($kind, $sections, $data, $meta);
        ...
        // ------------------------------------------------------------
        // DEBUG (local): dump the final HTML fed to TCPDF (PDF only)
        // Toggle by creating a file at the project root: .debug_pdf_html
        // Output: api/lib/__debug_pdf_output.html (+ timestamped copies)
        // ------------------------------------------------------------
        if ($kind === 'pdf') {
            $togglePath = dirname(__DIR__, 2) . '/.debug_pdf_html';
            if (is_file($togglePath)) {
                $html = str_replace(
                ...
                $p2 = __DIR__ . '/__debug_pdf_output.' . $ts . '.html';

                // Best-effort write; do not break PDF generation if this fails.
                @file_put_contents($p1, $html);
                @file_put_contents($p2, $html);
